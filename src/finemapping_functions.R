...
run_CARMA <- function(sumstats, ld, sumstats_name, window_mb, locus, ld_pop, LDpanel){
  z.list<-list()
  ld.list<-list()
  lambda.list<-list()
  z.list[[1]]<-sumstat$Z
  ...
  lambda.list[[1]]<-1 
  CARMA.results<-CARMA(z.list, ld.list, lambda.list=lambda.list, outlier.switch=TRUE)
...
run_susie <- function(sumstats, LDmat, N_tot, N_cases, ld_pop, sumstats_name, window_mb, locus, LDpanel, coverage = 0.95){
...
get_ss_per_locus <- function(df_chrom, LOCUS, START, END){
